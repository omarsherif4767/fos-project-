...
struct Env* fos_scheduler_MLFQ()
{
	...
	{
		int new_level;
		if(last_level == num_of_ready_queues-1)
		{
			new_level=num_of_ready_queues-1;
		}
		else
		{
	...
	if(chosen_level!=-1)
	{
		struct Env*next=dequeue(&(env_ready_queues[chosen_level]));
		kclock_set_quantum(quantums[chosen_level]);
