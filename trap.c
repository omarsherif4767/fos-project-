...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	fault_va=ROUNDDOWN(fault_va,PAGE_SIZE); //
	 unsigned int ws_size=env_page_ws_get_size(  curenv);
	unsigned int ws_max_size= curenv->page_WS_max_size;
	uint32 target_index;
	if(ws_size<ws_max_size){
		// in case of cold fault
	 for(int i=0;i<ws_max_size;i++){
		...
		// WARM FAULT
		 //setting to required N
		uint32 hand=0;
		uint32 victim_index=0;
		int found=0;
		while(1)
		{
			if(found && hand==ws_max_size)
			{
				break;
			}
			if(hand==ws_max_size)
			...
			uint32 page_va=env_page_ws_get_virtual_address(curenv,hand);
			uint32 perms=pt_get_page_permissions(curenv,page_va);
			if(perms&PERM_USED){
				pt_set_page_permissions(curenv,page_va,0,PERM_USED);
				curenv->ptr_pageWorkingSet[hand].sweeps_counter=0;

			}
			else{
				if(	curenv->ptr_pageWorkingSet[hand].sweeps_counter< page_WS_max_sweeps)
				{
					curenv->ptr_pageWorkingSet[hand].sweeps_counter++;

				}
				if(curenv->ptr_pageWorkingSet[hand].sweeps_counter==page_WS_max_sweeps && found==0)
					{victim_index=hand;
					found=1;
				...
				//hand=(hand+1)%ws_max_size;

                 hand++;

		}
		uint32 victim_va=env_page_ws_get_virtual_address(curenv,victim_index); //updating page FILE
		uint32 victim_perms=pt_get_page_permissions(curenv,victim_va);
		if(victim_perms& PERM_MODIFIED){
			uint32 * victim_pt;
			struct Frame_Info* victim_frame=get_frame_info(curenv->env_page_directory,(void*)victim_va,&victim_pt);
		...
		if(ret==E_PAGE_NOT_EXIST_IN_PF){
			if(fault_va>=USTACKBOTTOM && fault_va< USTACKTOP){
				...
				panic(" Page not in page file and not a stack page!!");
			}
		}
		env_page_ws_set_entry(curenv,target_index,fault_va);
		curenv->page_last_WS_index=(target_index+1)%ws_max_size;
